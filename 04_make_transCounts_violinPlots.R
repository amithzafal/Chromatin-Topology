...
{
    #print(time)
    result <- c(time,as.numeric(quantile(data[data$time==time,]$transCounts, c(.25, .50, .75), na.rm = TRUE)))
    #print(result)
...
outFile <- paste0("transCounts.pdf")
if(!file.exists(outFile))
{
    print(paste0("outFile ",outFile))

    p <- ggplot(data, aes(x=factor(time), y=transCounts, fill=factor(time))) +
	   ...
	   labs(x="",y="trans-chromosome counts",fill="") +     
	   guides(fill="none")
