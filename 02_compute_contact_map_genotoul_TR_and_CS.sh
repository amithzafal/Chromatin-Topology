	...
	do
		...
		do
			...
			do
			    ...
			    # Checking if the result of Compartment strength analysis is present!
			    outFile=${outMatrix%.tab}_0_obsOverExp_CSHomoOverAll.tab
			    if [[ -e ${outFile} ]];
			    then
				ls -lrtha ${outFile}
				continue
				...
				~/TOOLS/compute_contact_map_TR_and_CS -r ${res} -p ${np} -d ${rc} -b ${b} -k ${ncopies} -i 0
